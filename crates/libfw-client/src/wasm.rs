//! WASM-specific glue layer for the browser client.
//!
//! This module intentionally keeps the JS/browser bindings separate from the
//! reusable client logic in the rest of the crate. The business logic lives in
//! the library modules (`config`, `download`, `upload`, `plan`, `state`, `tune`)
//! while this file only handles `wasm-bindgen` exposure and browser callbacks.
...
impl LibfwClient {
    ...
    pub fn download_folder(&self, base_url: &str, token: &str, dir_path: &str) -> js_sys::Promise {
        ...
        wasm_bindgen_futures::future_to_promise(async move {
            ...
            match crate::download::download_folder(
                &base_url,
                &token,
                &dir_path,
                &callbacks,
...
async fn prepare_transfer(
    ...
    let caps = if config.auto_tune {
        match crate::http::fetch_capabilities(base_url, config.timeout_ms).await {
            Ok(c) => c,
    ...
    let level = crate::tune::resolve_level(config.compress_level, &caps);
